...
class PdoVanille
{
	...
	public function getProduitNutriscore($id)
	{
		$req = "select nutriScore from produit where PDT_id = '$id";
		$res = PdoVanille::$monPdo->query($req);
		$nutri = $res->fetch();
	...
	public function ajouterProduit($description, $prix, $image, $idCategorie, $nutri)
	{
		switch ($idCategorie)
		{
			case 'bon' : {
		...
		$req = "select count * from produit where idCategorie = '$idCategorie'";
		$res = PdoVanille::$monPdo->query($req);
		$lesLignes = $res->fetchAll();
		$id = $id + ($lesLignes+1);

		$req = "INSERT into produit VALUES ($id, $description, $prix, $image, $idCategorie, $nutri)";
		$res = PdoVanille::$monPdo->exec();
	}
